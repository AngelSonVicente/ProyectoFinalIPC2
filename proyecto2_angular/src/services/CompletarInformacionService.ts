...
export class CompletarInformacionService {
  ...
  public actualizarCurriculum(codigoUsuario: string, archivo: File): Observable<CompletarPerfilUsuario> {
    const formData: FormData = new FormData();
    formData.append('codigoUsuario', JSON.stringify(codigoUsuario));
    formData.append('CV', archivo, archivo.name);


    return this.httpClient.put<CompletarPerfilUsuario>(this.API_URL + 'Curriculum', formData);
  }
